...
export default async function JobsPage({
  ...
  const { q, type, location } = await searchParams; //value en el search params y "q" es la query
  ...
  const jobs = await prisma.job.findMany({
    where: {
      AND: [
        //Estamos buscando por jobs,type y location por eso el AND
        query
          ? {
              OR: [
                // contiene sin destinguir minisculas o mayusculas
                { title: { contains: query, mode: "insensitive" } }, // por el titulo
                { company: { contains: query, mode: "insensitive" } }, // por la empresa
                { description: { contains: query, mode: "insensitive" } }, // por la descripcion
              ],
            }
          : {}, //si no tiene nada el query, esta vacio
